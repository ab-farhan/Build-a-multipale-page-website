    ...
    <div class="row">
        ...
        @foreach ($courses as $data)
            <div class="col-md-4 thumbnail-container">
                <img src="{{ asset('/'.$data->course_img)}}" alt="Avatar" class="thumbnail-image ">
                <div class="thumbnail-middle">
                    <h1 class="thumbnail-title"> {{$data->course_name}} </h1>
                    <h1 class="thumbnail-subtitle">{{$data->course_sort_des}} </h1>
                    <a href="{{$data->course_link}}" target="_blank" class="normal-btn btn">শুরু করুন</a>
                </div>
            </div> 
        @endforeach
